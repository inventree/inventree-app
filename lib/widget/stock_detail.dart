...
class _StockItemDisplayState extends RefreshableState<StockDetailWidget> {
  ...
  final _changeStatusKey = GlobalKey<FormState>();
  ...
  void _editStockItem(BuildContext context) async {
    ...
    item.editForm(context, L10().editItem, fields: fields,
  ...
  void _changeStatusDialog() async {
    ...
    showFormDialog(
      ...
      fields: <Widget>[
        // Text("Current status: ${item.statusLabel(context)}"),
  ...
  void _addStock() async {
    ...
    final bool result =
        await item.addStock(context, quantity, notes: _notesController.text);
  ...
  void _removeStock() async {
    ...
    final bool result =
        await item.removeStock(context, quantity, notes: _notesController.text);
  ...
  void _countStock() async {
    ...
    final bool result =
        await item.countStock(context, quantity, notes: _notesController.text);
  ...
  void _countStockDialog() async {
    ...
    showFormDialog(L10().countStock, key: _countStockKey, callback: () {
      _countStock();
    }, acceptText: L10().count, fields: <Widget>[
      QuantityField(
  ...
  void _unassignBarcode(BuildContext context) async {
    ...
    if (result) {
      showSnackIcon(L10().stockItemUpdateSuccess, success: true);
    } else {
      showSnackIcon(
  ...
  void _transferStock(int locationId) async {
    double quantity =
        double.tryParse(_quantityController.text) ?? item.quantity;
    ...
    var result =
        await item.transferStock(locationId, quantity: quantity, notes: notes);
  ...
  Widget headerTile() {
    return Card(
      ...
      trailing: Text(item.statusLabel(context),
  ...
  List<Widget> actionTiles(BuildContext context) {
    ...
    tiles.add(ListTile(
      ...
      leading: FaIcon(FontAwesomeIcons.edit, color: COLOR_CLICK),
      onTap: _changeStatusDialog,
    ));
    ...
    tiles.add(ListTile(
      ...
      onTap: () {
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => InvenTreeQRView(
                    StockItemScanIntoLocationHandler(item)))).then((context) {
          refresh();
        });
      },
    ));
      ...
      tiles.add(ListTile(
          ...
          onTap: () {
            Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) => InvenTreeQRView(
                            StockItemBarcodeAssignmentHandler(item))))
                .then((context) {
              refresh();
            });
          }));
  ...
  Widget getSelectedWidget(int index) {
    switch (index) {
      case 0:
        return ListView(
          children: ListTile.divideTiles(context: context, tiles: detailTiles())
              .toList(),
        );
      case 1:
        return ListView(
            children: ListTile.divideTiles(
                    context: context, tiles: actionTiles(context))
                .toList());
      default:
        return ListView();
    }
